...
public:
    ...
    // 이코드 에서 사용할거
	int kind_of_shape = 0;
    int first_draw_face = -1,second_draw_face = -1, third_draw_face = -1;
    bool face = false;
    // 이 코드에서 사용할거
    ...
    void face_draw() {
        for (int i = 0;i < indices.size() / kind_of_shape;i++) {
            if (i != first_draw_face && i != second_draw_face && i != third_draw_face) continue;
            glBindVertexArray(vao);
...
Shape ground = {
    {
        { glm::vec3(-10.5f, -5.0f,  10.5f), glm::vec4(0.549f, 0.549f, 0.549f, 1.0f) }, // 0
        { glm::vec3(10.5f, -5.0f,  10.5f), glm::vec4(0.549f, 0.549f, 0.549f, 1.0f) }, // 1
        { glm::vec3(10.5f, -5.0f, -10.5f), glm::vec4(0.549f, 0.549f, 0.549f, 1.0f) }, // 2
        { glm::vec3(-10.5f, -5.0f, -10.5f), glm::vec4(0.549f, 0.549f, 0.549f, 1.0f) } // 3
    },
    {
        0, 1, 2, // 첫 번째 삼각형
        2, 3, 0  // 두 번째 삼각형
...
Shape pyramid = {
	{
		{ glm::vec3(0.0f,  1.0f,  0.0f), glm::vec4(1.0f, 0.0f, 0.0f, 1.0f) }, // 꼭대기 (Red)
		{ glm::vec3(-1.0f, -1.0f,  1.0f), glm::vec4(1.0f, 0.0f, 0.0f, 1.0f) }, // 앞 왼쪽 (Green)
		{ glm::vec3(1.0f, -1.0f,  1.0f), glm::vec4(1.0f, 0.0f, 0.0f, 1.0f) }, // 앞 오른쪽 (Blue)

        { glm::vec3(0.0f,  1.0f,  0.0f), glm::vec4(0.0f, 1.0f, 0.0f, 1.0f) }, // 꼭대기 (Red)
        { glm::vec3(-1.0f, -1.0f, -1.0f), glm::vec4(0.0f, 1.0f, 0.0f, 1.0f) }, // 뒤 왼쪽 (Magenta)
        { glm::vec3(-1.0f, -1.0f,  1.0f), glm::vec4(0.0f, 1.0f, 0.0f, 1.0f) }, // 앞 왼쪽 (Green)

        { glm::vec3(0.0f,  1.0f,  0.0f), glm::vec4(0.0f, 0.0f, 1.0f, 1.0f) }, // 꼭대기 (Red)
		{ glm::vec3(1.0f, -1.0f, -1.0f), glm::vec4(0.0f, 0.0f, 1.0f, 1.0f) }, // 뒤 오른쪽 (Yellow)
		{ glm::vec3(-1.0f, -1.0f, -1.0f), glm::vec4(0.0f, 0.0f, 1.0f, 1.0f) }, // 뒤 왼쪽 (Magenta)

        { glm::vec3(0.0f,  1.0f,  0.0f), glm::vec4(1.0f, 0.0f, 1.0f, 1.0f) }, // 꼭대기 (Red)
		{ glm::vec3(1.0f, -1.0f,  1.0f), glm::vec4(1.0f, 0.0f, 1.0f, 1.0f) }, // 앞 오른쪽 (Blue)
		{ glm::vec3(1.0f, -1.0f, -1.0f), glm::vec4(1.0f, 0.0f, 1.0f, 1.0f) },  // 뒤 오른쪽 (Yellow)

        { glm::vec3(1.0f, -1.0f,  1.0f), glm::vec4(0.5f, 0.0f, 0.5f, 1.0f) }, // 앞 오른쪽 (Blue)
		{ glm::vec3(-1.0f, -1.0f,  1.0f), glm::vec4(0.5f, 0.0f, 0.5f, 1.0f) }, // 앞 왼쪽 (Green)
	    { glm::vec3(-1.0f, -1.0f, -1.0f), glm::vec4(0.5f, 0.0f, 0.5f, 1.0f) }, // 뒤 왼쪽 (Magenta)

        { glm::vec3(1.0f, -1.0f,  1.0f), glm::vec4(0.5f, 0.0f, 0.5f, 1.0f) }, // 앞 오른쪽 (Blue)
		{ glm::vec3(-1.0f, -1.0f, -1.0f), glm::vec4(0.5f, 0.0f, 0.5f, 1.0f) }, // 뒤 왼쪽 (Magenta)
		{ glm::vec3(1.0f, -1.0f, -1.0f), glm::vec4(0.5f, 0.0f, 0.5f, 1.0f) }  // 뒤 오른쪽 (Yellow)
	},
	{
		0,1,2,
		3,4,5,
